...
{
  public partial class yachts02 : System.Web.UI.Page
  {
    ...
    private void BindYactLayout()
    {
      using (SqlConnection conn = new SqlConnection(config))
      {
        using (SqlCommand cmd = new SqlCommand("SELECT layout FROM [YachtDetail] WHERE yacht_id = @id", conn))
        {
          cmd.Parameters.AddWithValue("@id", Request.QueryString["id"] ?? "1");
          conn.Open();
          SqlDataReader dr = cmd.ExecuteReader();
          while (dr.Read())
          {
            Literal1.Text = dr["layout"].ToString();
          }
          dr.Close();
        }
      }
